...
class OrderBookWidget(QWidget):
    ...
    def updateData(self,data):
        ...
        tradingAsksValues = [ ]
        for v in data['asks']:
            tradingAsksValues.append(int(v['price'] * v['quantity']))
        maxTradingAsksValue = max(tradingAsksValues)
        ...
        for i, v in enumerate(data['asks'][::-1]):
            ...
            item_2.setRange(0,maxTradingAsksValue)
            item_2.setFormat(f"{tradingAsksValues[i]:,}")
            item_2.setValue(tradingAsksValues[i])
